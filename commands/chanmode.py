		...
		if len(arg) == 1:
			if arg[0].startswith("#"):
				if self.getflag(source, arg[0]) == "n" or self.getflag(source, arg[0]) == "q" or self.getflag(source, arg[0]) == "a":
					for channel in self.query("select name,modes from channelinfo where name = '{0}'".format(excape_string(arg[0]))):
			...
			if arg[0].startswith("#"):
				if self.getflag(source, arg[0]) == "n" or self.getflag(source, arg[0]) == "q" or self.getflag(source, arg[0]) == "a":
					for channel in self.query("select name from channelinfo where name = '{0}'".format(excape_string(arg[0]))):
						modes = self.regexflag(channel["flags"], modes, True)
						self.query("update channelinfo set modes = '{0}' where name = '{1}'".format(excape_string(modes), channel["name"]))
