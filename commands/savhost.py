		...
		if len(arg) == 0:
			...
			if arg[0] == "?list":
				...
				for data in self.query("select user,vhost from vhosts where active = '0' and user = '%s'" % arg[0]):
					...
					self.query("insert into memo (`user`, `source`, `message`) values ('%s', '%s', 'Your vHost %s has been activated.')" % (data["user"], self.bot_nick, data["vhost"]))
			...
			if arg[0] == "?set":
				if self.user(arg[1]):
					...
					if not entry:
						...
						self.query("insert into memo (`user`, `source`, `message`) values ('%s', '%s', '%s has been set as your vHost')" % (self.user(arg[1]), self.bot_nick, _mysql.escape_string(arg[2])))
				...
				if self.user(arg[1]):
					...
					self.query("delete from vhosts where user = '%s'" % arg[1])
					self.query("insert into memo (`user`, `source`, `message`) values ('%s', '%s', 'Your vHost %s has been deleted.')" % (self.user(arg[1]), self.bot_nick, vhost))
				...
				for data in self.query("select * from vhosts where active = '0' and user = '%s'" % arg[0]):
					...
					self.query("insert into memo (`user`, `source`, `message`) values ('%s', '%s', 'Your vHost %s has been rejected. Reason: %s')" % (data["user"], self.bot_nick, data["vhost"], _mysql.escape_string(' '.join(arg[1:]))))
					self.memo(data[0])
		else:
