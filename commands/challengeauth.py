		...
		if self.auth(uid) == 0:
			...
			if len(arg) == 3:
				...
				for challenges in self.query("select challenge from challenges where hostmask = '%s'" % _mysql.escape_string(self.hostmask(uid)[0])):
					...
					for data in self.query("select name,pass,suspended from users where name = '%s'" % user):
						if data["suspended"] == "0":
							if alg.lower() == "hmac-md5" and user == data["name"]:
								...
								if response == resp:
									correct = True
							elif alg.lower() == "hmac-sha-1" and user == data["name"]:
								...
								if response == resp:
									correct = True
							elif alg.lower() == "hmac-sha-256" and user == data["name"]:
								...
								if response == resp:
									correct = True
							elif alg.lower() == "hmac-sha-512" and user == data["name"]:
							...
							self.msg(uid, "Your account has been banned from " + self.services_description + ". Reason: " + data["suspended"])
